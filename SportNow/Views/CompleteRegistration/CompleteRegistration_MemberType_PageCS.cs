...
{
    public class CompleteRegistration_MemberType_PageCS : DefaultPage
	{

        Picker memberTypePicker, isFederadoPicker;
        Label memberTypeLabel, isFederadoLabel;
        ...
        {
            Debug.Print("selectCorrectItemFederadoPicker " + App.member.federado_tipo);
            int selectedIndex = 0;
            isFederadoPicker.SelectedIndex = 1;
            foreach (var federadoType in Constants.federadoTypeList)
            {
                Debug.Print("selectCorrectItemFederadoPicker " + federadoType.Key);
                if (App.member.federado_tipo.Contains(federadoType.Key))
                {
                    isFederadoPicker.SelectedIndex = selectedIndex;
                    break;
        ...
        public CompleteRegistration_MemberType_PageCS()
		{
            this.initLayout();
        ...
        {
            //SAVE SPORTS!!!!!
            App.member.socio_tipo = Constants.memberTypeList.KeyByValue(memberTypePicker.SelectedItem.ToString());
            App.member.federado_tipo = Constants.federadoTypeList.KeyByValue(isFederadoPicker.SelectedItem.ToString());
